...
highestGpa unis
|length ([y\\y<-:unis])==0="No universities given"
= (last (sort [y\\y<-:unis])).uniName
...
studentGpa stud= toReal(sum [y\\y<-:(stud.grades)])/(toReal (length [y\\y<-:stud.grades]))
...
fun [] [] =[]
fun a b
|length a>=length b=[x*y\\ x<-a & y<-b] ++ [x\\x<-( drop (length b) a)]
=fun b a
