...
getLastName per = {x\\ x<-lname}
    where
        lname = tl lstnamewithspace
        lstnamewithspace = dropWhile (\x = x<>' ') longnames
        longnames = [x\\x<-:per.lastName]

// Start = getLastName {givenName = "Pedro Henrique", lastName = "Villar deFigueiredo", gender = Male}
extractNode :: FamilyTree -> Person
extractNode (Name x _ _) = x

concatLastNames :: String String -> String
concatLastNames mname fname = mname +++ " " +++ fname
// concatLastNames mname fname = lastname
//     where
//         lastname = {x\\x<-list}
//         list = [x\\x<-:mname] ++ [' '] ++ [x\\x<-:fname]

fixOne :: FamilyTree -> Person
fixOne (Name per r l)
| right.gender == Male = extractNode (Name {per& lastName = llstn} r l)
= extractNode (Name {per& lastName = rlstn} r l)
    where
        rlstn = concatLastNames rightname leftname
        llstn = concatLastNames leftname rightname
        rightname = getLastName right
        leftname = getLastName left
        right = extractNode r 
        left = extractNode l 

// Start = fixOne PedroFamily
fixLastNames :: FamilyTree -> FamilyTree
fixLastNames (Name per End End) = (Name per End End)
fixLastNames (Name per l r) = (Name (fixOne (Name per (fixLastNames l) (fixLastNames r))) (fixLastNames l) (fixLastNames r))
